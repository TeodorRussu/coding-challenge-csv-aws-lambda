...
def find_tariffs(input_data):
    ...
    for entry in input_list:
        ...
        tariffs_for_date = data_dict.get(matching_db_date)
        power = find_power_value_in_the_input_item(entry)
        matching_tariff = find_matching_tarrif_from_row(power, tariffs_for_date)

        total_result = total_result + matching_tariff
    return total_result
